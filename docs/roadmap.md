...
readable here. The journal is done -- `kernel/fs/ext3.c` writes and replays
it -- but it covers metadata only, and it checkpoints every transaction
instead of batching, which costs more than it needs to.
