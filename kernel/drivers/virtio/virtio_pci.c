...
static uint64_t bar_address(const struct pci_device *pci, unsigned index) {
    if (index >= 6U) return 0;
    uint32_t low = pci->bar[index];
    if (low & 1U) return 0;
    uint64_t address = low & 0xFFFFFFF0U;
    if (((low >> 1U) & 3U) == 2U) {
        if (index + 1U >= 6U) return 0;
        address |= (uint64_t)pci->bar[index + 1U] << 32;
    }
    return address;
}
...
static volatile uint8_t *map_bar(const struct pci_device *pci, unsigned index) {
    ...
    uint64_t physical = bar_address(pci, index);
