...
/* Whether the processor promises a TSC that counts at a constant rate and does
   not stop, which is what every deadline and every slice here assumes. */
int time_tsc_is_invariant(void);
/* This processor's own reading of the clock, taken as it comes up, so that the
   processor that started it can check the two agree. */
void time_mark_processor(unsigned index);
/* Say so if that reading did not fall between the two the starter took around
   it, which is the only way from here to see clocks that disagree. */
