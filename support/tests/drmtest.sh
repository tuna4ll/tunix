...
# Build a machine whose whole userland is the DRM test, boot it, and print what it found.
...
cc -std=gnu11 -Wall -Wextra -Werror -O2 -static -nostdlib -nostartfiles \
	...
	\
	support/tests/drmtest.c -o "$WORK/root/sbin/init"
...
for _ in $(seq 240); do
	grep -q "DRMTEST DONE" "$LOG" 2>/dev/null && break
...
grep -E "^(LATENCY|DRMTEST|VERSION|CAP|CLIENTCAP|PLANE|ATOMIC|BLOB|ISOLATION|FBLIFETIME|OVERFLOW|MODESET)" "$LOG" || {
