...
work="$(mktemp -d /tmp/tunix-proctest.XXXXXX)"
...
cc -std=gnu11 -Wall -Wextra -Werror -O2 -static -nostdlib -nostartfiles \
    ...
    support/tests/proctest.c -o "$work/root/sbin/init"
...
timeout 120 qemu-system-x86_64 -machine "q35,accel=$accel" -cpu "$cpu" \
    -smp "${CPUS:-2}" -m 1G -drive "format=raw,file=$work/tunix.img,if=none,id=disk0" \
...
for unused in $(seq 120); do
    grep -q 'PROCTEST DONE' "$work/serial.log" 2>/dev/null && break
    kill -0 "$qemu_pid" 2>/dev/null || break
    sleep 1
done
kill "$qemu_pid" 2>/dev/null || true
wait "$qemu_pid" 2>/dev/null || true
if ! grep '^PROCTEST' "$work/serial.log"; then
    tail -80 "$work/serial.log"
    exit 1
fi
grep -q '^PROCTEST PASS$' "$work/serial.log"
