...
[ "$ARCH" != x86_64 ] || [ -x "$LIMINE_DIR/limine" ] || { echo "schedbench: limine is missing; run make kernel" >&2; exit 1; }
...
$CC -std=gnu11 -Wall -Wextra -Werror -O2 -static -nostdlib -nostartfiles \
	...
	-DTUNIX_BENCH_CPUS="$CPUS" support/tests/schedbench.c -o "$WORK/root/sbin/init" || exit 1
...
for _ in $(seq "$WAIT"); do
	grep -aq "BENCH DONE" "$LOG" 2>/dev/null && break
...
grep -aE "^(BENCH|NICE|QUANTUM|WAKE|SWITCH|PARALLEL|SLEEPER|SOCKWAIT|ACCEPT)" "$LOG" || {
