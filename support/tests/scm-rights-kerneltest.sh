...
work="$(mktemp -d /tmp/tunix-scm-rights-kerneltest.XXXXXX)"
...
cc -std=gnu11 -Wall -Wextra -Werror -O2 -static -nostdlib -nostartfiles \
    ...
    support/tests/scm-rights-kerneltest.c \
...
for unused in $(seq 120); do
    grep -q 'SCMRIGHTSTEST' "$work/serial.log" 2>/dev/null && break
    kill -0 "$qemu_pid" 2>/dev/null || break
    sleep 1
done
kill "$qemu_pid" 2>/dev/null || true
wait "$qemu_pid" 2>/dev/null || true
if ! grep '^SCMRIGHTSTEST' "$work/serial.log"; then
    tail -80 "$work/serial.log"
    exit 1
fi
grep -q '^SCMRIGHTSTEST PASS$' "$work/serial.log"
